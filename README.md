...
当前实现与功能边界见 [项目功能总结](PROJECT_OVERVIEW.md)。远程只保留 `dev`（日常开发）和 `master`（主分支，GitHub 默认分支）；两个分支都包含完整的前端、后端和数据库代码。
...
- **活动列表**：首页提供「近期活动 / 往期活动」切换和“只看官方”筛选；首条活动使用精选大图卡，其余使用紧凑横卡。后端支持关键词、难度、出行方式筛选，当前首页未展示这些筛选入口
...
- **报名人员管理**：普通活动由发起人管理，官方活动由官方账号和超级管理员共同管理；支持管理取消、恢复管理取消的报名，并记录操作人和时间。恢复时检查剩余名额，用户无法自行恢复被管理取消的报名
- **人数上限**：普通活动最多 100 人，官方活动最多 200 人，名额统计包含同行人
...
- **回顾列表**：展示所有已发布的活动回顾
- **回顾详情**：图文展示活动精彩瞬间
- **新建回顾**：超级管理员从尚未创建回顾的官方活动中选择来源，系统自动带入名称、时间、地点、难度、里程、爬升和有效报名人数；基础信息可微调，活动总结支持富文本编辑，人合照、卜合照和公益记录图片单独上传
...
```bash
# 克隆完整项目开发分支；主分支使用 -b master
git clone -b dev https://github.com/lichengding0813/bubuchuhu.git
